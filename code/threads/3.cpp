...
void MergeSort(int* height, int* h, int n, int k, int budget) {
    if (n == 1) return;
    bool parallel = (n >= threshold && budget > 1);

    if (parallel == true) {
        thread t;
        t = thread(MergeSort, height, h, n / 2, k, budget / 2);
        MergeSort(height + n / 2, h + n / 2, n - n / 2, k, budget - budget / 2);
        t.join();
    }
    else {
        MergeSort(height, h, n / 2, k, 1);
        MergeSort(height + n / 2, h + n / 2, n - n / 2, k, 1);
    }
...
int main() {
    ...
    for (int alpha = 0; alpha < z; ++alpha) {
        ...
        MergeSort(height, buffer.data(), n, k, available_threads);
